...
# ==========================================
# 3. FLORENCE-2 ENCODER + TRANSFORMER DECODER
# ==========================================
...
def train():
    ...
    captions_dict = load_captions(os.path.join(DATA_DIR, "caption.txt"))

    print(f"Train images: {len(train_list)}")
    print(f"Val images: {len(val_list)}")
    print(f"Captioned images: {len(captions_dict)}")
    ...
    train_ds = BanglaCaptionDataset(train_list, captions_dict, vocab, os.path.join(DATA_DIR, "Pictures"), True)
    val_ds = BanglaCaptionDataset(val_list, captions_dict, vocab, os.path.join(DATA_DIR, "Pictures"), False)
    ...
    print(f"\nFLORENCE-2 + Transformer Decoder: {NUM_LAYERS} Layers | {HIDDEN_DIM} Hidden | OneCycle LR")
