...
MODEL_DIR = os.path.join(PROJECT_DIR, "models", "facebook-pixio-vitb16", "model")
OUTPUT_DIR = os.path.join(PROJECT_DIR, "models", "facebook-pixio-vitb16", "output")
...
class Encoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit = ViTModel.from_pretrained(MODEL_DIR, ignore_mismatched_sizes=True)
...
class Decoder(nn.Module):
    def __init__(self, embed_dim=768, hidden_dim=2048, vocab_size=10000, feature_dim=768, num_layers=6, nhead=12, dropout=0.1):
        super().__init__()
...
def test():
    ...
    checkpoint = torch.load(os.path.join(OUTPUT_DIR, "checkpoint_epoch_20.pt"), map_location=DEVICE)
