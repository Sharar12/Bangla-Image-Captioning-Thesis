...
def find_latest_checkpoint(output_dir):
    ...
    for f in os.listdir(output_dir):
        m = pattern.match(f)
        if m:
            checkpoints.append((int(m.group(1)), os.path.join(output_dir, f)))
    if not checkpoints:
        return None
...
def find_existing_log(output_dir):
    candidates = [f for f in os.listdir(output_dir) if f.endswith(".xlsx")]
    if not candidates:
        return None
    candidates.sort()
...
def train_continue():
    ...
    result = find_latest_checkpoint(OUTPUT_DIR)
    if result is None:
        print(f"No checkpoint found in {OUTPUT_DIR}. Run train.py first.")
        return
    checkpoint_path, checkpoint_epoch_num = result
    print(f"Found checkpoint: {checkpoint_path} (epoch {checkpoint_epoch_num})")

    log_file = find_existing_log(OUTPUT_DIR)
    if log_file is None:
        log_file = os.path.join(
            OUTPUT_DIR, f"training_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
    else:
        print(f"Continuing existing log: {log_file}")
    ...
    # Handle both old format (without scheduler) and new format (with scheduler)
    model.load_state_dict(checkpoint["model_state_dict"])
    optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
    loaded_epoch = checkpoint["epoch"]

    # Fast-forward scheduler to match completed epochs
    opt_steps_taken = (loaded_epoch + 1) * steps_per_epoch
    for _ in range(opt_steps_taken):
        scheduler.step()

    print(f"Resumed from epoch {loaded_epoch + 1} (scheduler steps: {opt_steps_taken})")
