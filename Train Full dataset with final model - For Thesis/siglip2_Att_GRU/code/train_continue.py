...
def train_continue():
    ...
    if os.path.exists(vocab_path):
        ...
        captions_raw = load_captions(os.path.join(DATA_DIR, "captions.txt"))
        vocab = Vocabulary()
    ...
    if latest is not None:
        ...
        if completed_steps < scheduler.total_steps:
    ...
    print(f"\n{'=' * 50}")
    print(f"Architecture: SigLIP2 → Luong Attention + GRU")
    print(
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        try:
            if os.path.exists(log_file):
                pd.concat(
                    [pd.read_excel(log_file), df_new], ignore_index=True
