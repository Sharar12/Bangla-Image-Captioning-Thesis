...
def train():
    ...
    train_dataset = BanglaCaptionDataset(train_list, captions, vocab, processor, os.path.join(DATA_DIR, "Pictures"))
    val_dataset = BanglaCaptionDataset(val_list, captions, vocab, processor, os.path.join(DATA_DIR, "Pictures"))
    ...
    optimizer = torch.optim.AdamW(model.decoder.parameters(), lr=LEARNING_RATE, weight_decay=0.01)
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Time: {epoch_time:.1f}s | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | LR: {scheduler.get_last_lr()[0]:.6f}\n")
