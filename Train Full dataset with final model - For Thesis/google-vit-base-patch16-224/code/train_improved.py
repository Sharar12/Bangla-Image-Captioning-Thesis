...
# --- High IQ Configuration ---
EMBED_DIM = 768               # Matches ViT-Base
NHEAD = 12                    # 64 units per head
NUM_LAYERS = 10               # Increased Depth
HIDDEN_DIM = 4096             # Massive Thinking Space
DROPOUT = 0.3                # Stronger regularization
LABEL_SMOOTHING = 0.1         # Forces flexible word choice
...
class BanglaCaptionDataset(Dataset):
    def __init__(self, image_list, captions, vocab, image_dir, is_train=True):
        ...
        # High-speed torchvision transforms (ViT standard)
...
class Decoder(nn.Module):
    def __init__(self, embed_dim=EMBED_DIM, hidden_dim=HIDDEN_DIM, vocab_size=10000, 
                 feature_dim=768, num_layers=NUM_LAYERS, nhead=NHEAD, dropout=DROPOUT):
        super().__init__()
...
# ==========================================
# 4. TRAINING LOOP WITH ONE-CYCLE LR
# ==========================================
...
def train():
    ...
    print(f"\nULTRA IQ MODE: {NUM_LAYERS} Layers | {HIDDEN_DIM} Hidden | OneCycle LR")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Time: {epoch_time:.1f}s | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | LR: {current_lr:.6f}\n")
