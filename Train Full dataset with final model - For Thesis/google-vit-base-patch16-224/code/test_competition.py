...
def calculate_cider(reference_list, hypothesis):
    ...
    for ref_tokens in ref_tokens_list:
        ...
        for n in [1, 2, 3, 4]:
            ...
            sim = cosine_similarity(ref_ngrams, hyp_ngrams, len(ref_tokens), len(hyp_tokens))
            score += sim
...
def test():
    ...
    checkpoint = torch.load(os.path.join(OUTPUT_DIR, "checkpoint_epoch_10.pt"), map_location=DEVICE)
    ...
    avg_cider = sum(all_metrics['cider']) / len(all_metrics['cider']) if all_metrics['cider'] else 0.0
