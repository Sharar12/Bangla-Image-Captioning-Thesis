...
def load_captions(caption_file):
    ...
    for _, row in df.iterrows():
        captions[row['virtual_filename']].append(row['caption'])
    return captions
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        for param in self.siglip.vision_model.encoder.layers[-4:].parameters():
...
def train():
    ...
    start_epoch = 0
    checkpoint_files = [f for f in os.listdir(OUTPUT_DIR) if f.startswith("best_epoch_") and f.endswith(".pt")]
    if checkpoint_files:
        checkpoint_files.sort(key=lambda x: int(x.replace("best_epoch_", "").replace(".pt", "")))
    ...
    print(f"Vocab Filter: min_freq={MIN_FREQ} | Vision Fine-tuning: Last 4 layers unfrozen")
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"🌟 New Best Validation Loss at Epoch {epoch+1}: {best_val_loss:.4f}")
        ...
        }, os.path.join(OUTPUT_DIR, f"best_epoch_{epoch + 1}.pt"))
