...
USE_AMP = True
NUM_WORKERS = 8          # <--- KEY FIX: no extra processes → NO MemoryError
MAX_SEQ_LEN = 32

FEATURE_DIM = 768
EMBED_DIM = 768
NHEAD = 12  # 64 units/head (Standard)
NUM_LAYERS = 8  # Judge the encoder, save 60% time
HIDDEN_DIM = 6144  # Reduce math overhead
DROPOUT = 0.1
LABEL_SMOOTHING = 0.1
MIN_FREQ = 64  # Small vocab = Faster final layer
...
class BanglaCaptionDataset(Dataset):
    def __init__(self, image_list, captions, vocab, image_dir, is_train=True):
        ...
        # All transforms work on CPU tensors here (we'll move them to GPU in the training loop if you want)
        # For simplicity and stability, we keep them on CPU and let the DataLoader handle transfer.
        if is_train:
...
def train():
    ...
    train_dataset = BanglaCaptionDataset(train_list, captions, vocab,
                                         os.path.join(DATA_DIR, "images"), is_train=True)
    val_dataset = BanglaCaptionDataset(val_list, captions, vocab,
                                       os.path.join(DATA_DIR, "images"), is_train=False)

    # No persistent_workers because num_workers=0
    ...
    print(f"NUM_WORKERS: {NUM_WORKERS}  ← crash‑free mode\n")
...
if __name__ == "__main__":
    # On Windows, force spawn to avoid any conflicts (already default)
