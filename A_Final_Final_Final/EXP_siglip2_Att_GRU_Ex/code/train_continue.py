...
def save_training_log(log_file, epoch, train_loss, val_loss, lr, time_taken):
    ...
    try:
        ...
        print(f"  [log] Excel save failed ({e}), falling back to CSV")
...
def train_continue():
    ...
    n_val = min(int(n * VAL_RATIO), n)
    n_test = min(int(n * TEST_RATIO), n)

    train_idx = perm[:n_train]
    # If train uses all data, val/test still get their shares from the start
    # (overlap is intentional — user wants full-data training while still
    #  having validation and test splits for evaluation)
    if n_train >= n:
        val_idx = perm[:n_val]
    ...
    if latest is not None:
        ...
        if checkpoint is None:
            ...
            ckpt_vocab_size = checkpoint["model_state_dict"][
                "decoder.gru.embedding.weight"
            ].size(0)
            if ckpt_vocab_size != vocab.vocab_size:
                vocab_changed = True
                print(
                    f"WARNING: Checkpoint vocab ({ckpt_vocab_size}) != current vocab ({vocab.vocab_size}). "
                    f"Dataset changed. Reinitializing embedding/output layers."
                )
                keys_to_skip = {
                    "decoder.gru.embedding.weight",
                    "decoder.banglagpt.embedding.weight",
                    "decoder.banglagpt.output_proj.weight",
                    "decoder.banglagpt.output_proj.bias",
                }
                filtered = {
                    k: v
    ...
    if (
        ...
        and not vocab_changed
    ):
        optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
    elif vocab_changed:
        print("Skipping optimizer state (vocab size changed)")
    ...
    print(f"LR: Cosine schedule with warmup ({warmup_steps} warmup steps, peak=5e-5)")
    print(f"{'=' * 50}\n")
