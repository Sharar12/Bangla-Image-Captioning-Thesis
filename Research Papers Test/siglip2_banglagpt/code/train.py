...
# ==============================================================================
# FORCE CRITICAL OS ENVIRONMENT BLOCKS (BEFORE ANY OTHER IMPORTS)
# ==============================================================================
# This forces PyTorch's underlying C++ and Python worker engines to completely
# silence anything below a critical hard error across all multiprocessing sub-processes.
...
# Suppress warnings inside the main process thread
...
def download_models():
    ...
    if not os.path.exists(BANGLA_GPT_PATH):
        ...
        tokenizer.save_pretrained(BANGLA_GPT_PATH)
        model.save_pretrained(BANGLA_GPT_PATH)
        print(f"BanglaGPT saved to {BANGLA_GPT_PATH}")
    else:
        print(f"BanglaGPT already exists at {BANGLA_GPT_PATH}")

    return SIGLIP_MODEL_PATH, BANGLA_GPT_PATH
...
class CaptionModel(nn.Module):
    ...
    @torch.no_grad()
    def generate_caption(self, pixel_values, tokenizer, max_new_tokens=MAX_SEQ_LEN, repetition_penalty=1.2):
        self.eval()
        ...
        suppress_ids = {tokenizer.pad_token_id, tokenizer.unk_token_id}
        ...
        for step in range(max_new_tokens):
            ...
            vis_mask = torch.ones(batch_size, num_vis, device=device, dtype=torch.long)
            text_mask = torch.ones(batch_size, input_ids.size(1), device=device, dtype=torch.long)
            ...
            for b in range(batch_size):
                if finished_mask[b]:
                    continue
                seen_tokens = set(generated_tokens[b])
            ...
            for b in range(batch_size):
                if finished_mask[b]:
                    continue
                t_id = next_tokens[b].item()
                if t_id == eos_id:
                    finished_mask[b] = True
                else:
                    generated_tokens[b].append(t_id)
    ...
    def generate_captions(self, pixel_values, tokenizer, max_new_tokens=MAX_SEQ_LEN):
        return [self.generate_caption(pixel_values[i].unsqueeze(0), tokenizer, max_new_tokens) for i in
                range(pixel_values.size(0))]
        ...
        for batch_idx, (pixel_values, input_ids, attention_mask) in enumerate(pbar):
            ...
            if USE_AMP:
                ...
                if (batch_idx + 1) % ACCUMULATION_STEPS == 0:
                    ...
                    scaler.update()
                    optimizer.zero_grad()
                    scheduler.step()  # FIX: Stepping scheduler safely AFTER optimizer step
            else:
                loss.backward()
                if (batch_idx + 1) % ACCUMULATION_STEPS == 0:
                    torch.nn.utils.clip_grad_norm_(all_params, max_norm=1.0)
