...
class Encoder(nn.Module):
    def forward(self, pixel_values):
        return self.swin(pixel_values).last_hidden_state
...
class CaptionModel(nn.Module):
    ...
    def generate_captions(self, pixel_values, tokenizer, max_new_tokens=MAX_SEQ_LEN):
        ...
        generated = torch.full(
            (pixel_values.size(0), 1), bos_id, dtype=torch.long, device=DEVICE
        )
...
def run_evaluation():
    ...
    print("Building model (Swin-tiny -> BanglaBERT with cross-attention)...")
    model = CaptionModel()
    ...
    print("Loading Swin-tiny vision encoder...")
    ...
    print("Loading BanglaBERT decoder with cross-attention...")
    ...
    print("Loading Swin image processor...")
    processor_path = (
        swin_path
