...
VIT_MODEL_PATH = os.path.join(MODELS_DIR, "vit-base-patch16-224")
...
def download_models():
    ...
    if not os.path.exists(VIT_MODEL_PATH):
        print("Downloading vit-base-patch16-224...")
        model = AutoModel.from_pretrained(
            "google/vit-base-patch16-224",
            trust_remote_code=True,
        )
        model.save_pretrained(VIT_MODEL_PATH)
        processor = AutoImageProcessor.from_pretrained("google/vit-base-patch16-224")
        processor.save_pretrained(VIT_MODEL_PATH)
        print(f"ViT saved to {VIT_MODEL_PATH}")
    else:
        print(f"ViT already exists at {VIT_MODEL_PATH}")
    ...
    return VIT_MODEL_PATH, BANGLA_BERT_PATH
...
class Encoder(nn.Module):
    def __init__(self, unfreeze_layers=UNFREEZE_VISION_LAYERS):
        ...
        self.vit = AutoModel.from_pretrained(model_path, trust_remote_code=True)
...
def train():
    ...
    unfrozen_vision = sum(p.requires_grad for p in model.encoder.vit.parameters())
    vision_status = (
        "Fully Frozen"
    ...
    print(f"\nArchitecture: ViT-base Vision -> BanglaBERT Decoder")
