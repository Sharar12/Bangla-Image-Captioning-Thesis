...
class Encoder(nn.Module):
    ...
    def forward(self, pixel_values):
        vision_attr = [v for k, v in self._modules.items() if "vit" in k]
        if vision_attr:
            return vision_attr[0](pixel_values).last_hidden_state
        return (
            getattr(self, list(self._modules.keys())[0])
            ._modules["vit"](pixel_values)
            .last_hidden_state
        )
...
class CaptionModel(nn.Module):
    ...
    def generate_captions(self, pixel_values, tokenizer, max_new_tokens=MAX_SEQ_LEN):
        ...
        stop_token_id = bangla_full_stop_id[0] if bangla_full_stop_id else None
...
def run_evaluation():
    ...
    print("Loading ViT-base vision encoder...")
    vit = AutoModel.from_pretrained(vit_path, trust_remote_code=True)
    for param in vit.parameters():
        param.requires_grad = False
    setattr(model.encoder, "vit", vit)
    ...
    processor_path = (
        vit_path
        if os.path.exists(os.path.join(vit_path, "preprocessor_config.json"))
        else "google/vit-base-patch16-224"
    )
